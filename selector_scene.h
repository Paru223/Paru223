...
public:
	...
	void on_draw(const Camera& camera)
	{
		...
		// 玩家1（左侧）背景绘制（带滚动效果）
		putimage_alpha(camera,
			selector_background_scroll_offset_x - img_p1_selector_background->getwidth(),
			0,
		...
		putimage_alpha(
			...
			0,
			img_p1_selector_background->getwidth() - selector_background_scroll_offset_x,
			0,
			img_p1_selector_background,
			0,
			0);
		...
		// 玩家2（右侧）背景绘制（带滚动效果）  
		putimage_alpha(
			getwidth() - img_p2_selector_background->getwidth(),
			0,
			img_p2_selector_background->getwidth() - selector_background_scroll_offset_x,
			0,
		...
		putimage_alpha(camera,
			getwidth() - selector_background_scroll_offset_x,
			0,
			img_p2_selector_background);
		...
		case PlayerType::Peashooter:
			...
			outtextxy(pos_img_1P_name.x, pos_img_1P_name.y, str_peashooter_name);
			break;
		case PlayerType::Sunflower:
			...
			outtextxy(pos_img_1P_name.x, pos_img_1P_name.y, str_sunflower_name);
			break;
		}
		...
		case PlayerType::Peashooter:
			...
			outtextxy(pos_img_2P_name.x, pos_img_2P_name.y, str_peashooter_name);
			break;
		case PlayerType::Sunflower:
			animation_sunflower.on_draw(camera, pos_animation_2P.x, pos_animation_2P.y);
			pos_img_2P_name.x = pos_img_2P_gravestone.x + (img_gravestone_right.getwidth() - textwidth(str_peashooter_name)) / 2;
			outtextxy(pos_img_2P_name.x, pos_img_2P_name.y, str_sunflower_name);
			break;
		}
